...
public class LookUpTable {
    ...
    public void add(double... row) {
        if (row.length != size) {
            System.err.println("Size of new row (" + row.length + ") does not match row size of: " + size);
            return;
        }
        table.add(row);
    ...
    public double[] get(double value) {
        ...
        if (table.isEmpty()) {
            System.out.println("Cannot interpolate - table is empty. Use add() to add data first.");
            return null;
        }
