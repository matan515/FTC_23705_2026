...
@Autonomous(name = "RedAutoshoothome (Blocks to Java)", preselectTeleOp = "Main")
public class RedAutoshoothome extends LinearOpMode {
        ...
        if (opModeIsActive()) {
            while (opModeIsActive()) {
                if (autoStateMachine.equals(LAUNCH_BALLS)) {
                    ...
                    if (myElapsedTime.seconds() < 1.5) {
                        ((DcMotorEx) left_drive).setVelocity(72);
